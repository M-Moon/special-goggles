...
class Client(tk.Frame):
    ...
    def __init__(self, parent, width=400, height=500):  # init app
        ...
        Client.get_config(self) # starting by getting config, or setting up new one
    ...
    def enter_pressed(self):  # called if enter is pressed
        ...
        if len(input_get) > 0:
            Client.update_new_message(self, self.user, input_get)  # adding the text to the chat window
            if self.connected:
                self.cnct.send_message(input_get)  # sending the message to the other client

    def update_new_message(self, name, text):  # updating chat window
    ...
    def update_message_other(self, text):
        self.messages.config(state='normal')  # make text box configurable
    ...
    def handle_incoming_msg(self): # checking to update text inside gui as opposed to in connection class
        while True: # running forever as thread
            try:
                if self.cnct: # if the connection object exists
                    if self.cnct.incoming_msg: # if the variable for incoming messages exists
                        if self.cnct.incoming_msg != None: # if the variable contains a message
                            Client.update_new_message(self, self.cnct.other_name, self.cnct.incoming_msg) # updating textbox with received message
                            self.cnct.incoming_msg = None # setting variable to empty again
    ...
    def establish_connection(self, ip, port):  # attempting to establish connection with other client
        ...
        if int(port) < 1024:
            ...
            try: # attempt the connection
                ...
                Client.update_message_other(self, "Connection established with {} on port {}\n".format(ip, port))
    ...
    def break_connection(self): # function to break connection
        ...
        Client.update_message_other(self, "Successfully disconnected.") # confirming connection disconnected
